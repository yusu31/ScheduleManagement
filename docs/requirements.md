...
| F-11 | 地図表示 | イベント詳細ページに地図を表示する（OpenStreetMap 等） | 住所から地図が表示される | 🚧 未実装 |
...
| 用途 | 方針 |
|------|------|
| 全体のトーン | 地域密着感のある温かみのある配色 |
| メインカラー | グリーン系（自然・福島らしさ）またはオレンジ系（活気・賑わい） |
...
| 画像 | 各イベントに画像URLを登録できる。画像未設定時はカテゴリに応じたデフォルト画像を表示 |
| 一覧レイアウト | グリッドレイアウト（Instagram 風）でイベントを並べ、スクロールで発見できる体験 |
...
| Phase 3 | 発展機能（F-10〜F-11）の実装 | 本格的なポートフォリオに仕上げる |
