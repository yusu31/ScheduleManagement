...
| F-03 | ユーザー認証 | サインアップ・ログイン・ログアウトができる | 認証なしではお気に入り登録・予定管理ができない | 🚧 未実装 |
| F-04 | カレンダー表示・予定管理 | FullCalendar でイベントを月・週単位で表示する。Connpass から取得したイベントをカレンダーに予定として追加して予定管理ができる | イベントがカレンダー上に表示され、クリックで詳細に遷移できる | 🚧 未実装 |

### Phase 2 ― 追加機能（Phase 1 完成後に追加）

| # | 機能名 | 詳細 | 受け入れ条件 | 実装状況 |
|---|--------|------|-------------|---------|
| F-05 | 絞り込み・ソート検索 | 地域・カテゴリ・日付（開始〜終了）でイベントを絞り込める。新着順・開催日順でソートできる | 条件を指定すると該当イベントのみ表示され、ソート順を切り替えられる | 🚧 未実装 |
| F-06 | Connpass 自動取得 | Connpass API から福島関連イベントを取得して DB に保存する | 重複取得を防ぎ、取得したイベントが一覧に自動表示される | 🚧 未実装 |
| F-07 | 今週末特集枠 | トップページに「今週末のイベント」を自動抽出して特集表示する | 今週の土日に開催されるイベントが特集枠に自動表示される | 🚧 未実装 |
| F-08 | 天気予報バッジ | イベント当日の天気を OpenWeatherMap API から取得し、イベントカードに天気アイコンを表示する | イベントカードにその日の天気アイコン（晴・曇・雨など）が表示される | 🚧 未実装 |
| F-09 | ファミリー向けフィルター | 「子連れOK」「無料」「室内」タグでイベントを絞り込める | ファミリー向けタグを選択すると該当イベントのみ表示される | 🚧 未実装 |
...
| # | 機能名 | 詳細 | 受け入れ条件 | 実装状況 |
|---|--------|------|-------------|---------|
| F-11 | お気に入り機能 | ログイン後、イベントをお気に入りに登録・解除できる | お気に入り一覧ページで登録したイベントを確認できる | 🚧 未実装 |
| F-12 | 近くの食事処 | イベント詳細ページに開催場所周辺のグルメ情報を表示する（HotPepper グルメ API） | イベント詳細ページに周辺レストラン一覧が表示される | 🚧 未実装 |
| F-13 | 地図表示（詳細ページ） | イベント詳細ページに地図を表示する（Leaflet.js + OpenStreetMap） | 開催場所の住所から地図上にピンが表示される | 🚧 未実装 |
| F-14 | 地図でイベントを探す | マップページで福島県内のイベントをピンで一覧表示する。ピンをクリックすると写真プレビューと詳細リンクが表示される（Mori Fes 参考） | 地図上のピンからイベント詳細に遷移できる | 🚧 未実装 |

### Phase 4 ― オリジナル機能（Phase 3 完成後に追加）

| # | 機能名 | 詳細 | 受け入れ条件 | 実装状況 |
|---|--------|------|-------------|---------|
| F-15 | マップ制覇機能 | 福島全59市町村を制覇するゲーム性のある機能。イベント参加後に写真＋誰と行ったか（家族/恋人/一人/友人）を記録する。市町村の形に写真をトリミングして地図上に表示（SVGクリッピング）。未訪問は灰色、訪問済みは写真で埋まっていく | 市町村をタップして写真と記録を登録できる。訪問済み市町村が写真で表示される | 🚧 未実装 |
